...
resource "aws_ecs_task_definition" "web" {
  ...
  container_definitions    = templatefile("${path.module}/tasks/web_task_definition.json", {
    image           = "${aws_ecr_repository.order_app.repository_url}"
    db_username = "${var.database_username}"
    db_password = "${var.database_password}"
    db_host    = "${var.dbhost}"
    database_name   = "${var.database_name}"
    log_group       = "${aws_cloudwatch_log_group.order.name}"
    ...
    output_payment_status_notification_url = "${var.output_payment_status_notification_url}"
...
resource "aws_alb" "alb_order" {
  name            = "${var.prefix}-alb-order"
...
resource "aws_alb_listener" "order" {
  load_balancer_arn = "${aws_alb.alb_order.arn}"
  port              = "3000"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.prefix}_order_web_cpu_utilization_high"
